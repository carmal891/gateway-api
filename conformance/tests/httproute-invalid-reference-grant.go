...
var HTTPRouteInvalidReferenceGrant = suitepkg.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suitepkg.ConformanceTestSuite) {
		routeNN := types.NamespacedName{Name: "reference-grant", Namespace: suitepkg.InfrastructureNamespace}
		...
		t.Run("Simple HTTP request not should reach web-backend", func(t *testing.T) {
			http.MakeRequestAndExpectEventuallyConsistentResponse(t, suite.RoundTripper, suite.TimeoutConfig, gwAddr, http.ExpectedResponse{
				...
				Backend:   "web-backend",
				Namespace: suitepkg.WebBackendNamespace,
			})
		})
	},
}
