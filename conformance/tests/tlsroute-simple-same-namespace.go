...
var TLSRouteSimpleSameNamespace = suitepkg.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suitepkg.ConformanceTestSuite) {
		...
		routeNN := types.NamespacedName{Name: suitepkg.InfrastructureGatewayName, Namespace: ns}
		...
		t.Run("Simple TLS request matching TLSRoute should reach infra-backend", func(t *testing.T) {
			tcp.MakeTCPRequestAndExpectEventuallyValidResponse(t, suite.TimeoutConfig, gwAddr, serverCertPem, serverStr, true,
				tcp.ExpectedResponse{
					...
					Backend:      "tcp-backend",
					Namespace:    suitepkg.InfrastructureNamespace,
					Hostname:     serverStr,
				})
		})
	},
